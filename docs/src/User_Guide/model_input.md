...
##### Operational binning beyond VRE

The same `Num_VRE_bins` column can now also be used to cluster capacity-investment bins for the following resource types: **Thermal** (commit and no-commit), **Storage** (symmetric, asymmetric, long-duration), **Must-run**, **Flexible demand**, **Electrolyzer**. Hydro and VRE-storage hybrid resources are **not** supported.

Semantics are identical to the VRE case: the first row of a cluster declares `Num_VRE_bins = N` (N consecutive bins), and the next N-1 rows declare `Num_VRE_bins = 0`. The cluster shares a single set of operational variables (power output, commitment, storage state of charge, charging, deferred demand, etc.); each bin contributes its own capacity decision (and where applicable, its own availability profile) to the operational constraints of the first bin.

Key rules:

- **Default value of 1** for resource types where the column is missing — existing inputs without the column behave identically to before (single-bin, single-resource).
- **Cluster structure**: bins of a cluster must be **consecutive resource IDs**, **in the same zone**, and **of the same resource type and subtype** (e.g., all symmetric storage, or all thermal-commit). The first row declares `Num_VRE_bins = N`; the next N-1 rows declare `Num_VRE_bins = 0`. The clustering is not validated — mixed-type clusters will produce silently incorrect results.
- **Operating parameters come from the first bin.** Min power, ramp rates, efficiencies, `cap_size`, min up/down times, `tau_advance`/`tau_delay`, hydrogen efficiency, derating factors, etc. are taken from the first-bin row of the cluster. Non-first-bin values for these parameters are ignored.
- **Variable O&M is the first bin's.** Investment cost (and energy/charge cost where applicable) is per-bin, so bins represent different cost tiers of an otherwise identical resource.
- **Capacity-derived contributions are per-bin.** Each bin contributes independently to: investment cost, fixed O&M, min/max capacity constraints, MinCapReq/MaxCapReq tags, thermal CRM (derated nameplate × per-bin capacity).
- **Outputs**: the first bin reports the cluster's full operational quantities (vP, vCHARGE, vS, etc.) on its row; non-first bins show zeros for operational outputs. Capacity outputs (Existing/New/End capacity, retirements) remain per-bin.
- **Profiles in `Generators_variability.csv`**: every bin needs its own column for any resource type whose capacity constraint uses a time-varying `pP_Max` — this includes **thermal no-commit, must-run, flexible demand, electrolyzer** (and VRE itself). The cluster's bound is computed as `Σ pP_Max[yy,t] · eTotalCap[yy]` across bins, so a missing column for a non-first bin defaults to `pP_Max = 1.0` and silently over-credits that bin's capacity. If you want all bins to share the same profile (the typical case for non-VRE clusters), duplicate the column under each bin's resource name. **Storage** does not use `pP_Max` in its cluster bounds and does not require per-bin profiles. **Thermal commit** uses only the first bin's `pP_Max` (per-unit formulation with `vCOMMIT`), so per-bin profiles are not summed there — the first bin's profile applies to the whole cluster.
- **Incompatible features**: do not enable `Num_VRE_bins > 1` together with fusion plants or maintenance scheduling on the same resource. The fusion and maintenance modules iterate over all bins and the resulting model behavior is undefined.
