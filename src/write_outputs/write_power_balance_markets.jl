function write_power_balance_markets(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    if setup["Markets"] == 1
        push!(Com_list, "Market_Buy")
        push!(Com_list, "Market_Sell")
    end


    L = length(Com_list)
    Com_idx = Dict(Com_list[i] => i for i in 1:L)
    ...
    for z in 1:Z
        ...
        powerbalance[(z - 1) * L + Com_idx["Generation"], :] = sum(value.(EP[:vP][POWER_ZONE, :]), dims = 1)
        if !isempty(intersect(resources_in_zone_by_rid(gen, z), STOR_ALL))
            ...
            powerbalance[(z - 1) * L + Com_idx["Storage_Discharge"], :] = sum(value.(EP[:vP][STOR_ALL_ZONE, :]),
                dims = 1)
            powerbalance[(z - 1) * L + Com_idx["Storage_Charge"], :] = (-1) * sum(
        ...
        if !isempty(intersect(resources_in_zone_by_rid(gen, z), FLEX))
            ...
            powerbalance[(z - 1) * L + Com_idx["Flexible_Demand_Defer"], :] = sum(
                (value.(EP[:vCHARGE_FLEX][FLEX_ZONE,:]).data),
                dims = 1)
            powerbalance[(z - 1) * L + Com_idx["Flexible_Demand_Stasify"], :] = (-1) *
                                               sum(value.(EP[:vP][FLEX_ZONE, :]), dims = 1)
        end

        if SEG > 1
            powerbalance[(z - 1) * L + Com_idx["Demand_Response"], :] = sum(value.(EP[:vNSE][2:SEG, :, z]), dims = 1)
        end
        powerbalance[(z - 1) * L + Com_idx["Nonserved_Energy"], :] = value.(EP[:vNSE][1, :, z])
        if Z >= 2
            powerbalance[(z - 1) * L + Com_idx["Transmission_NetExport"], :] = (value.(EP[:ePowerBalanceNetExportFlows][:,
                z]))' # Transpose
            powerbalance[(z - 1) * L + Com_idx["Transmission_Losses"], :] = -(value.(EP[:eLosses_By_Zone][z, :]))
        end
        powerbalance[(z - 1) * L + Com_idx["Demand"], :] = (((-1) * inputs["pD"][:, z]))' # Transpose
        if !isempty(ELECTROLYZER)
            ELECTROLYZER_ZONE = intersect(resources_in_zone_by_rid(gen, z), ELECTROLYZER)
            powerbalance[(z - 1) * L + Com_idx["Electrolyzer_Consumption"], :] = (-1) * sum(
    ...
    if setup["WriteOutputs"] == "annual"
        ...
        CSV.write(joinpath(path, "power_balance_markets.csv"),
